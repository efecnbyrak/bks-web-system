...
export default async function KuralPage({
    ...
    const [rules, readProgress] = await Promise.all([
        ...
        db.ruleProgress.findMany({
            where: { userId: session.userId, type: "kural" },
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="bg-white dark:bg-zinc-950 rounded-3xl border border-zinc-200 dark:border-zinc-800 shadow-xl overflow-hidden">
                <div className="px-4 sm:px-6 pt-5 sm:pt-6 pb-4 sm:pb-5 border-b border-zinc-100 dark:border-zinc-900">
                    <div className="flex items-center gap-3">
                        <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-red-600 flex items-center justify-center shadow-md flex-shrink-0">
                            <Book className="w-4 h-4 sm:w-5 sm:h-5 text-white" />
                        </div>
                        <div className="min-w-0">
                            <h2 className="text-base sm:text-lg font-black text-zinc-900 dark:text-white tracking-tight">Basketbol Oyun Kuralları 2024</h2>
                            <p className="text-xs text-zinc-500 font-medium">Tüm maddeler · Arama yapın veya listede gezinin</p>
                        </div>
                    </div>
                </div>
                <div className="p-4 sm:p-6">
                    <NativeRulesViewer type="kural" readArticleIds={readArticleIds} />
                </div>
            </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        {rules.map((rule) => {
                            ...
                            return (
                                <Link
                                    key={rule.id}
                                    href={`/referee/bag/rules/kural?id=${rule.id}`}
                                    scroll={false}
